...
use FireflyIII\Support\Import\Routine\Plaid\StageAuthenticatedHandler;
use FireflyIII\Support\Import\Routine\Plaid\StageImportDataHandler;
use Log;

/**
 * Class PlaidRoutine
 */
class PlaidRoutine implements RoutineInterface
{
    ...
    {
        Log::debug(sprintf('Now in PlaidRoutine::run() with status "%s" and stage "%s".', $this->importJob->status, $this->importJob->stage));
        $valid = ['ready_to_run']; // should be only ready_to_run
        if (\in_array($this->importJob->status, $valid, true)) {
            switch ($this->importJob->stage) {
                default:
                    throw new FireflyException(sprintf('PlaidRoutine cannot handle stage "%s".', $this->importJob->stage)); // @codeCoverageIgnore
